...
import { Lock, LogOut, ArrowRight, ChevronDown } from 'lucide-react'
...
// Gated by the inventory module's own auth until hub auth is unified: a staff
// or admin account must be signed in at /inventory/auth.
export default function InventoryAdminArea() {
  ...
  if (!user || user.role === 'user') {
    return (
      <div className="inv-root flex min-h-[60vh] items-center justify-center">
        <div className="max-w-sm rounded-2xl border border-gray-200 bg-white p-8 text-center">
          ...
          <h2 className="m-0 mb-2 text-lg font-bold text-charcoal">Inventory staff sign-in required</h2>
          <p className="m-0 mb-6 text-sm text-inv-muted">
            The inventory module uses its own account system for now. Sign in with a staff or admin account to manage stock, requests, and payments.
          </p>
          <Link to="/inventory/auth"
            className="inline-flex items-center gap-2 rounded-xl border-none px-5 py-2.5 text-sm font-bold text-white"
            style={{ background: '#0F172A' }}>
            Sign in to Inventory <ArrowRight size={14} />
          </Link>
        </div>
      </div>
    )
  ...
  const signOut = () => { setUser(null); navigate('/inventory') }
