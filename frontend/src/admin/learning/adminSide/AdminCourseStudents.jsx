  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Enrolled students and interactive-path purchases.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6 max-w-md">
        <div className="bg-white rounded-xl border border-border p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-foreground tabular-nums">{students.length}</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-border p-4 flex items-center gap-3">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden md:table-cell">Interactive path</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr><td colSpan={4} className="px-5 py-8 text-center text-sm text-muted-foreground">Loading students…</td></tr>
              ) : students.length === 0 ? (
                <tr><td colSpan={4} className="px-5 py-8 text-center text-sm text-muted-foreground">No students enrolled yet.</td></tr>
              ) : students.map((s) => (
                <tr key={s.id} className="hover:bg-muted transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-muted-foreground hidden sm:table-cell">{s.email}</td>
                  <td className="px-5 py-3.5 text-muted-foreground whitespace-nowrap">{formatDate(s.enrolledAt)}</td>
