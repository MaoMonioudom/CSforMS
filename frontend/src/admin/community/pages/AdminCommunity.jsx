          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden lg:table-cell">Tags</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden sm:table-cell">Engagement</th>
              ...
              {list.map(post => (
                <tr key={post.id} className="hover:bg-muted transition-colors">
                  ...
                  <td className="px-5 py-3.5 hidden sm:table-cell">
                    <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full ${categoryColors[post.category] ?? "bg-muted text-muted-foreground"}`}>
                      {post.category}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 hidden md:table-cell">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="text-foreground truncate max-w-[110px] text-xs font-medium">{post.author.name}</p>
                        <p className="text-muted-foreground text-xs">{post.author.handle}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 hidden lg:table-cell">
                    <div className="flex flex-wrap gap-1">
                      {(post.tags ?? []).slice(0, 3).map(t => (
                        <span key={t} className="inline-block bg-muted text-muted-foreground text-xs px-1.5 py-0.5 rounded">
                          #{t}
                        </span>
                      ))}
                    </div>
