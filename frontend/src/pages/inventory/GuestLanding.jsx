...
export default function LandingPage({ onEnter, onBrowse, items = [], users = [], borrows = [] }) {
  ...
  return (
    <div style={{ background: CREAM, color: DARK, fontFamily: "'Poppins',Inter,system-ui,sans-serif", minHeight: "100vh" }}>
      ...
      {/* ── NAV ─────────────────────────────────────────────────────── */}
      <nav className="gap-2.5 px-4 sm:gap-4 sm:px-8 lg:px-12" style={{
        position: "sticky", top: 0, zIndex: 50,
        background: "rgba(255,255,255,0.96)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)",
        borderBottom: "1px solid #e2e8f0",
        display: "flex", alignItems: "center", justifyContent: "space-between", height: 64,
      }}>
        <img src={LOGO_IMAGE} alt="MakerVault" className="h-6 sm:h-7" style={{ width: "auto", objectFit: "contain", flexShrink: 0 }} />

        {/* Search — replaces Catalog / Zones / About links, always reachable */}
        <form onSubmit={submitSearch} className="mx-2 sm:mx-4" style={{ flex: 1, maxWidth: 420, position: "relative" }}>
          <Search size={13} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#94a3b8", pointerEvents: "none" }} />
          <input
            value={navQuery} onChange={e => setNavQuery(e.target.value)}
            placeholder="Search items…" aria-label="Search items"
            className="text-xs sm:text-[13px]"
            style={{
              width: "100%", background: "#f8fafc", border: "1.5px solid #e2e8f0", borderRadius: 999,
              color: DARK, padding: "8px 12px 8px 32px", outline: "none", transition: "border-color .15s, box-shadow .15s",
            }}
