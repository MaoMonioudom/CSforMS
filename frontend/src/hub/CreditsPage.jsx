            ...
            {REDEEM_ITEMS.map((r, i) => (
              <div key={r.label} className="flex items-center gap-3 px-4 py-3.5"
                ...
                <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: "color-mix(in oklch, var(--color-inv-accent) 10%, transparent)" }}>
                  <Package size={16} style={{ color: "var(--color-inv-accent)" }} />
                </div>
                <p className="flex-1 min-w-0 text-sm font-semibold" style={{ color: D.text }}>{r.label}</p>
                <span className="text-sm font-extrabold shrink-0" style={{ color: "var(--color-inv-accent)" }}>{r.cost} credits{r.unit}</span>
              </div>
            ))}
          </div>
        </div>
